\documentclass{article}

\usepackage{sectsty}      % for section heading

\usepackage{libertine}    % font
\usepackage[T1]{fontenc}  % for 8-bit encoding
\renewcommand{\familydefault}{\sfdefault}

\usepackage{enumitem}     % for customizing itemize
\usepackage{hyperref}     % for link
\usepackage{mdwlist}      % for compact lists (itemize*)

\usepackage[letterpaper,margin=1in]{geometry}

\sectionfont{\normalfont\sectionrule{0pt}{0pt}{-4pt}{1pt}}

% format two pieces of text: one left aligned and one right aligned
\newcommand{\headerrow}[2]
{\begin{tabular*}{\linewidth}{l@{\extracolsep{\fill}}r}
    #1 &
    #2 \\
\end{tabular*}}

\begin{document}
\thispagestyle{empty}

\begin{center}
    %{\huge \textsc{\textbf{Rahul Venkatesh}}} \\[2pt]
    {\huge \textsc{Rahul Venkatesh}} \\[2pt]
    % Paris $\mid$ +33 7 xx xx xx xx $\mid$ rahulv.9942@gmail.com
    +65 xxxx xxxx
    $\mid$
    rahulv.9942@gmail.com
    $\mid$
    linkedin.com/in/\href{https://www.linkedin.com/in/recurze}{recurze}
    $\mid$
    github.com/\href{https://github.com/recurze}{recurze}
\end{center}

\section*{\textsc{\textbf{Education}}}
\begin{itemize}[leftmargin=0em]

\item[]
    \headerrow {\textbf{National University of Singapore (NUS)}}{Singapore}
    \headerrow {\emph{Master of Science in Data Science and Machine Learning (GPA: 4.8/5)}}{\emph{Aug 2023 - Present}}

\item[]
    \headerrow {\textbf{Indian Institute of Technology (IIT), Delhi}}{New Delhi, India}
    \headerrow {\emph{Bachelor of Technology in Computer Science and Engineering (GPA: 7.37/10)}}{\emph{Jul 2016 - Jun 2020}}

\end{itemize}

%\section*{Scholastic Achievements}
%\begin{itemize}
%    \item
%        Selected for \textbf{ACM-ICPC $2018$} Regional Round with teams
%        competing from all over India.
%    \item
%        Secured \textbf{All India Rank - $2754$} in Joint Entrance Exam
%        (Advanced) - $2016$, among $150$k candidates.
%    \item
%        Awarded Kishore Vaigyanik Protsahan Yojana \textbf{(KVYP) Fellowship} by
%        Indian Institute of Science, $2015$.
%    \item
%        Received National Talent Search \textbf{(NTS) Scholarship} for being
%        in the top 1000 at National Level, $2012$.
%\end{itemize}
%

\section*{\textsc{\textbf{Technical Skills}}}
\begin{itemize}[itemsep=0em, leftmargin=0.7em]
\item[]
    \textbf{Languages}: Python, C++, SQL, Bash
\item[]
    \textbf{Tools/Software}: TensorFlow, MongoDB, MySQL, OpenMP, CUDA
\end{itemize}

\section*{\textsc{\textbf{Work Experience}}}
\begin{itemize}[leftmargin=0em]
\parskip=0.1em

\item[]
    \headerrow {\textbf{Squarepoint Capital}}{Paris, France}
    \headerrow {\emph{Software Engineer}}{\emph{Aug 2020 - Jun 2023}}

    \begin{itemize*}
        \item
            Developed and supported \textbf{low-latency} order entry gateways (OEG) for algorithmic trading in production.
        \item
            Built new OEGs to $5+$ \textbf{exchanges}, including \textbf{CME, ICE} and \textbf{OSE}
            over OUCH and FIX protocols.
        \item
            Designed frameworks for trading $2$ new asset classes:
            \textbf{bonds} and \textbf{non-deliverable forwards (NDF)}.
        \item
            \textbf{Improved performance} of gateway application by
            \textbf{collecting and analysing} latency-related \textbf{data}.
        \item
            Coordinated with QA and delivered numerous new projects (gateways) and business requests (features).
        \item
            Documented workflows and automations to streamline developing new gateways and production support.
    \end{itemize*}
\end{itemize}

\section*{\textsc{\textbf{Projects}}}
\begin{itemize}[leftmargin=0em]

\item[]
    \headerrow {\textbf{Option Pricing}}{}
    \begin{itemize*}
        \item
            Developed n-step \textbf{Binomial Option Pricing Model} as a discrete version of Black-Scholes model.
        \item
            Implemented \textbf{Least-Square Policy Iteration} (RL) to learn optimal exercise policy for American options.
        \item
            Applied LSTDQ to compute expected payoff upon continuation using Laguerre polynomials as \textbf{feature maps}.
        \item
            Computed \textbf{greeks} and conducted sensitivity analysis of the option price w.r.t the parameters $r, \tau,$ and $\sigma$.
        \item
            Used real data to derive risk-free rate and implied volatility, closely matching computed and market prices.
    \end{itemize*}

\item[]
    \headerrow {\textbf{Stock Price Prediction}}{}
    \begin{itemize*}
        \item
            Conducted time series analysis with \textbf{seasonal decomposition} and \textbf{stationarity} tests to identify patterns.
        \item
            Used \textbf{Moving Average} and \textbf{Exponential Smoothing} to approximate prices and remove noise.
        \item
            Implemented cross-validation and parameter grid search for model selection to reduce bias and overfitting.
        \item
            Applied diverse forecasting methods, including \textbf{ARIMA} (and VARMAX), and RNN like \textbf{LSTM}.
    \end{itemize*}


%\item[]
%    \headerrow {\textbf{MiniMovieDB}}{Prof. Yang Xiaoyan}
%    \headerrow {\emph{Database Project}}{\emph{Oct 2023 - Nov 2023}}
%
%    \begin{itemize*}
%        \item
%            Developed a \textbf{MongoDB} database for managing and storing information about shows, actors and user events.
%        \item
%            Designed \textbf{aggregation pipelines} to extract valuable insights, such as identifying popular shows and actors.
%        \item
%            Enhanced query performance by strategic \textbf{indexing} and optimized regex searches for faster data retrieval.
%        \item
%            Implemented \textbf{FastAPI} endpoints to search, filter, and serve data seamlessly to the web frontend.
%    \end{itemize*}

%\item[]
%    \headerrow {\textbf{Parallel Laplacian Solver}}{Prof. Amitabha Bagchi}
%    \headerrow {\emph{Algorithms Project}}{\emph{Aug 2019 - Dec 2019}}
%
%    \begin{itemize*}
%        \item
%            Implemented a novel \textbf{random-walk distributed} method to solve an
%            important class of Laplacian systems.
%        \item
%            Leveraged C++/OpenMP API for parallelization and gprof and valgrind for \textbf{profiling}.
%        \item
%            Improved performance on sparse graphs with Alias Method and "densifying" \textbf{optimizations}.
%        \item
%            Visualized performance against existing Dan Spielman's solver and Bechetti's solver: \\
%            Overall, it's \textbf{better than} Bechetti's, and with ample parameter fine-tuning, it's \textbf{comparable with} Dan's.
%    \end{itemize*}

%\item[]
%    \headerrow {\textbf{Public Credit Registry}}{Prof. Subhashis Banerjee, Prof.
%        Subodh Sharma}
%    \headerrow {\emph{Summer Undergraduate Project}}{\emph{May 2019 - Jul 2019}}
%
%    \begin{itemize*}
%        \item
%            Constructed first-order logical representation of data
%            privacy, security and other correctness properties.
%        \item
%            Proposed a complete example of a design with unit tests and
%            end-to-end testing of the model.
%        \item
%            Employed blockchain technology similar to \textbf{Hyperledger
%            Fabrics} and \textbf{Software Guard Extension}.
%    \end{itemize*}

%\item[]
%    \headerrow {\textbf{AI Game-playing bot}}{Prof. Mausam}
%    \headerrow {\emph{Artificial Intelligence Project}}{\emph{Oct 2018}}
%
%    \begin{itemize*}
%        \item
%            Created a bot to play Yinsh, a $2$-player game on a hexagonal board
%            with a branching factor of $30$.
%        \item
%            Applied \textbf{Alpha-beta pruning} with \textbf{static move ordering}
%            to choose next move to evaluate.
%        \item
%            Optimized move generation using \textbf{Bitboards} and \textbf{Zobrist Hashing},
%            boosting search to $4$ moves ahead.
%        \item
%            Fine-tuned weights for explicit hand-crafted features through automated trial-and-error.
%    \end{itemize*}

%\item
%    Toy prolog interpreter
%    $\mid$
%    Prof. Sanjiva Prasad
%    \hfill
%    MAR $2018$
%
%    \begin{itemize}
%        \item
%            Built a interpreter for logic programming language - Prolog, capable
%            of solving prolog predicates.
%        \item
%            The front end of the interpreter consitutes of the \textbf{Lex
%            scanner} (ocamllex) and \textbf{LALR parser} (ocamlyacc).
%        \item
%            The scanner converts the program into tokens which in turn are
%            converted into \textbf{Abstract Syntax Tree}.
%        \item
%            A explicit \textbf{backtracking algorithm} using fixed size stacks
%            was implemented to solve the predicates.
%    \end{itemize}

\end{itemize}

\section*{\textsc{\textbf{Relevant Coursework}}}
\begin{itemize}[itemsep=0em, leftmargin=0.7em]
\item[]
    Linear Algebra,
    Calculus,
    Probability and Stochastic Processes,
    Differential Equations,
    Optimization Algorithms for Data Modelling,
    Applied Regression Analysis\footnotemark{},
    Modelling and Numerical Simulations\footnotemark[\value{footnote}],
    Numerical Methods in Quantitative Finance\footnotemark[\value{footnote}]
\end{itemize}
\footnotetext{Currently enrolled}

%\section*{Positions of Responsibility}
%\begin{itemize}
%    \item
%        As Vice-Captain, led the hostel chess team to be placed first in
%        Inter-Hostel Chess Tournament 2017 at IIT.
%    \item
%        As Captain of the Red House, ensured we stood first in the annual
%        intra-high school extracurricular competitions.
%\end{itemize}

%\section*{\textsc{Interests, Talents and Skills}}
%\begin{itemize*}
%    \item
%        Chess - FIDE Rating: $1380$
%    \item
%        Basketball - Represented the school team and hostel team at IIT Delhi.
%    \item
%        Competitive programming (uname: recurze) – Ratings: Codeforces - $1813$
%        and Codechef - $1877$.
%\end{itemize*}

\end{document}
